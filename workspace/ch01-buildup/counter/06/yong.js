const yong = (() => {
  ...
  // createElement('button', {type: 'butto', onclick: 'handleUp()'}, '+');
  ...
  const createRoot = (rootNode) => {
    let _appComponent;
    return (_root = {
      // 루트노드 하위에 지정한 함수를 실행해서 받은 컴포넌트를 랜더링 한다.
      render(appFn) {
  ...
  const useState = (initValue) => {
    // 최초에 한번만 initValue 값으로 저장하고 useState가 다시 호출되면 initValue는 무시하고 저장한 값을 사용
    ...
    function setValue(newValue) {
      ...
      // 두 값이 같은지 비교해서 같지 않을 경우에(상태가 변경된 경우) 리렌더링
